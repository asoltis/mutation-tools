...
def main():
    ...
    # Loop over samples and compute info
    for sample in sample_names:
        ...
        OLINES = []
        of = open('%s%s_SVs.txt'%(outdir,sample),'w')
        of.writelines('\t'.join(['CHROM1','POS1','CHROM2','POS2','SV_type','SV_name','cluster'])+'\n')
        ...
        for i,cc in enumerate(ConnComp):
            ...
            for c in cc:
                ...
                OLINES.append([c.chrom1, c.pos1, c.chrom2, c.pos2, c.svtype, c.name, curr_clust])
        ...
        for SV in singletons:
            OLINES.append([SV.chrom1, SV.pos1, SV.chrom2, SV.pos2, SV.svtype, SV.name, curr_clust])
            curr_clust += 1
        
        print(' %s: %d total SVs - %d clusters with %d variants and %d singletons.'%(sample, len(SV_list), 
                                                                                     len(ConnComp), len(in_cluster), len(singletons)))

        # write output
        for OL in OLINES:
            of.writelines('%s\t%d\t%s\t%d\t%s\t%s\t%d\n'%(tuple(OL)))
        of.close()
